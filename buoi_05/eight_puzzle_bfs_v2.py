...
class PuzzleModel:
    ...
    def bfs_v2(self, start_matrix):
        node = Node(start_matrix, None, None, 0)
        if node.state == self.goal:
            return node, self.reached_count
            ...
            for action in self.get_actions(node.state):
                ...
                if self.tuple_matrix(child.state) not in explored and self.tuple_matrix(child.state) not in frontier_set:
                    if child.state == self.goal:
                        return child, self.reached_count
                    frontier.append(child)

        return False, self.reached_count

    def bfs_v2_solution(self, node):
        res = []
...
class PuzzleController:
    ...
    def run_bfs_thread(self, start_matrix):
        success, _ = self.model.bfs_v2(start_matrix)
    ...
    def on_solve_complete(self, success):
        ...
        if success:
            self.model.bfs_v2_solution(success)
